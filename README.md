...
**アカウントパスワードではなく、[アプリパスワード](https://bsky.app/settings/app-passwords)を設定・入力してログインしてください。**
...
SkyPutterはフォアグラウンドで一定時間おきにポーリングを行い、通知取得する仕組みです。

フォアグラウンド通知取得開始時は、Androidのサイレント通知が表示されます。

![Image](https://github.com/user-attachments/assets/7de77369-18e5-4c27-9014-ea3ec2a8ddba)

**アプリ終了(タスクトレイから削除)しても、フォアグラウンド動作は停止しません。**

以下の手段でこの動作を停止できます。

* 設定画面の「フォアグラウンド自動取得」をOFFにする
* 端末の電源OFF

### フォアグラウンド実行のリスクについて

デバイス仕様上、アプリのフォアグラウンド実行はバッテリー消費、データ通信量消費が大きくなります。
